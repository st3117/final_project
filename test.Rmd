...
alq_file_list = list.files("./data/") %>% 
  list.filter(str_detect(., "ALQ")) %>% 
  paste0("./data/", .)

_file_list = list.files("./data/") %>% 
  list.filter(str_detect(., "ALQ")) %>% 
  paste0("./data/", .)
    ...
    ifelse(str_detect(name, "DEMO") == TRUE, SELECT(SEQN, RIAGENDR, RIDAGEYR, RIDRETH3, DMDBORN4,
                       ...
                       ifelse(str_detect(name, "WHO") == TRUE, SELECT(), 
                              ifelse(str_detect(name, "SMQ") == TRUE, SELECT(SEQN, SMQ020),
                                     ifelse(str_detect(name, "ALQ") == TRUE, SELECT(SEQN, ALQ110)),
...
demo = 
  ...
  select(id = SEQN, gender = RIAGENDR, age = RIDAGEYR, race = RIDRETH3, edu = DMDEDUC3, six_month = RIDEXMON, 
...
alq = 
  map_dfr(alq_file_list, read_xpt_final) %>% 
  select(id = SEQN, alcohol = ALQ110, name) %>% 
  mutate(year = as.factor(ifelse(str_detect(name, "_G") == TRUE, "2011-2012", 
                       ifelse(str_detect(name, "_H") == TRUE, "2013-2014", "2015-2016")))) %>% 
...
# replce 
nhanes = merge(demo, merge(mcq, merge(whq, merge(smq, alq)))) %>% 
...
nhanes_model = 
  ...
  filter(gender == 2) %>% 
  select(id, year, age, race, breast_cancer, overweight, smoke_100, age_breast_cancer, alcohol, strata, psu, weight)
...
model2 = svyglm(breast_cancer ~ age + race + overweight + smoke_100, family = "binomial", design = design)
broom::tidy(model2)
summary(model2)
broom::glance(model2)
AIC(model2)
